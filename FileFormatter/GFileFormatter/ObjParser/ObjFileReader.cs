namespace GFileFormatter
{
    ...
    internal class ObjFileReader : IFileReader
    {
        /// <summary>
        /// The logger.
        /// </summary>
        ...
        private void CheckVertex(string line, ListBuffer<float> verticesBuffer)
        {
            if (line.StartsWith("v "))
            {
                ...
                float.TryParse(stringValues[1], out float vertexCoord);
                verticesBuffer.Add(vertexCoord);

                float.TryParse(stringValues[2], out vertexCoord);
                verticesBuffer.Add(vertexCoord);

                float.TryParse(stringValues[3], out vertexCoord);
                verticesBuffer.Add(vertexCoord);
            }
        }
        ...
        private void CheckTextureCoords(string line, List<float> textureCoords)
        {
            if (line.StartsWith("vt "))
            {
                ...
                float.TryParse(stringValues[1], out float textureCoord);
                textureCoords.Add(textureCoord);

                float.TryParse(stringValues[2], out textureCoord);
                textureCoords.Add(textureCoord);
            }
        }
        ...
        private void CheckNormal(string line, List<float> normals)
        {
            if (line.StartsWith("vn "))
            {
                ...
                float.TryParse(stringValues[1], out float normalCoord);
                normals.Add(normalCoord);

                float.TryParse(stringValues[2], out normalCoord);
                normals.Add(normalCoord);

                float.TryParse(stringValues[3], out normalCoord);
                normals.Add(normalCoord);
            }
        }
    }
